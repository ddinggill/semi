...
<div class="container" id="notice">
		...
		<table class="table table-striped"> 
			...
			<thead> 
		 	...
		 	<!-- 테이블 내용 -->
		 	<tbody> 
		 	<c:forEach items="${requestScope.aList }" var="aList" varStatus="status">
				<tr>
					<!--<td>${cnt }</td>-->
						<td>${pdto.number-status.count+1}</td>
						<td><a href="view.do?boardkey=${aList.boardkey }&pageNum=${pdto.currentPage}">${aList.fTitle }</a>
						...
						<td><input type="hidden" value="${aList.boardkey }"	name="boardkey"></td>
					</tr>
		</c:forEach>
		...
		<ul class="pagination">
			<c:if test="${requestScope.pdto.startPage != 1 }" >
				<li>
					<a href="board.do?pageNum=${requestScope.pdto.startPage- requestScope.pdto.blockPage}">이전</a>
				</li>
			</c:if>
				<li>
